...
public class SaveBoundedContextUseCase {

    final BoundedContextRepository repository;

    public void handle(SaveBoundedContextCommand command) {
        var role = repository.findById(new BoundedContextId(command.id())).orElseThrow();
        var bddScenarios = command.bddScenarios() == null ? List.<BddScenario>of() :
                command.bddScenarios().stream()
                        ...
                        .toList();
        role.update(new BoundedContextName(command.name()), command.gitRepository(),
