...
public class McpStdioServer {
    ...
    private ObjectNode initializeResult(JsonNode params) {
        ...
        result.put("instructions", "Authoring tools for the modux model store (the YAML spec that modux "
                ...
                + "1) topology — bootstrap_project creates project + service + boundedContexts (each with its main module, wired to the service) in one call from "
                + "the user's description; external systems live INSIDE the project element; "
                + "2) models first (data with shape), then escalate only when needed: add an aggregate "
                + "when there are invariants or a lifecycle to protect — and DECLARE those invariants "
                + "(the aggregate's 'invariants' list: they are its reason to exist); a query service or "
                + "read model for the read side, domain events for what happened, use cases for what the "
                + "system does; "
                + "3) relations between boundedContexts are declared as intent, not structure: use flows "
                + "(MATERIALIZES/TRIGGERS/NOTIFIES/ORCHESTRATES) or a recipe (list_recipes) — prefer "
                + "apply_recipe over hand-building the pieces. Cross-context orchestration is a WORKFLOW "
                + "(element type 'workflows'): steps form a dependency graph via dependsOnStepIds; a step "
                + "with roleId is a HUMAN task (deadline ISO-8601, escalationRoleId, compensationUseCaseId "
                + "to undo, formPageId = the page the forms engine renders); loose gateways live in "
                + "'workflowGateways' (JOIN n\u21921 with ALL/ANY, SPLIT 1\u2192n with PARALLEL/EXCLUSIVE "
                + "+ branchConditions); "
                + "4) operations are pipelines: gather data, transform (model mappings / 'transformations'), "
                + "write or return; hand-written pieces are 'customCodes', referenced from mappings, "
                ...
                + "(the AI surface), 'modules' + 'services' (distribution). "
