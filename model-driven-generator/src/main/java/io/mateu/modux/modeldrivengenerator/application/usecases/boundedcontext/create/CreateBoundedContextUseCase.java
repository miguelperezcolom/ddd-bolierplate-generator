package io.mateu.modux.modeldrivengenerator.application.usecases.boundedcontext.create;
...
public class CreateBoundedContextUseCase {

    final BoundedContextRepository repository;

    public void handle(CreateBoundedContextCommand command) {
        var bddScenarios = command.bddScenarios() == null ? List.<BddScenario>of() :
                command.bddScenarios().stream()
                        ...
                        .toList();
        var boundedContext = BoundedContext.of(new BoundedContextId(command.id()), new BoundedContextName(command.name()), command.gitRepository(),
                ...
                command.invariants() == null ? List.<Invariant>of() : command.invariants().stream()
                        .map(inv -> Invariant.of(new InvariantId(inv.id()), new InvariantName(inv.name()),
                                inv.conditions() == null ? List.of() : inv.conditions().stream()
                                        ...
                                        .toList()))
                        .toList());
        repository.save(boundedContext);
    }
