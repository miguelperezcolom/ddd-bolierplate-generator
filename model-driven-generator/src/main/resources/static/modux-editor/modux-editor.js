...
function lc() {
  ...
  f.transform = function(I, P, C, v) {
    ...
    $.property("__zoom", vo), I !== $ ? R(I, P, C, v) : $.interrupt().each(function() {
...
function Mc(e, t) {
  ...
  o.forEach((m, g) => {
    const x = t[m.id] ?? { x: 200 + g % 5 * 260, y: 160 + Math.floor(g / 5) * 220 }, d = m.fields ?? [], l = Eo + (d.length ? d.length * vi + (d.length - 1) * Mo : 10) + Ao;
    i.push({
      id: m.id,
      label: m.name,
      x: x.x,
